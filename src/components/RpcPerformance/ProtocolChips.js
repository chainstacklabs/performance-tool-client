      ...
      {chains.map((chain) => {
        ...
        return (
          <button
            ...
            onMouseEnter={e => { if (!isActive) e.currentTarget.style.borderColor = '#4A5260'; }}
            onMouseLeave={e => { if (!isActive) e.currentTarget.style.borderColor = '#2E3338'; }}
          >
            {logo && (
              <div style={{ width: 30, height: 30, position: 'relative', flexShrink: 0, borderRadius: 9999, overflow: 'hidden' }}>
                <Image src={`/logos/${logo}.svg`} alt={chain.name} fill style={{ objectFit: 'contain' }} />
              </div>
            )}
            <span style={{ position: 'relative', whiteSpace: 'nowrap' }}>
              <span style={{ fontSize: 15, lineHeight: '20px', fontWeight: 450, letterSpacing: '-0.14px', visibility: 'hidden', userSelect: 'none' }}>
                {chain.name}
              </span>
              <span style={{
