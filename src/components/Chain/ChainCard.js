...
export default async function ChainCard({ chain }) {
  ...
  return (
    <section
      ...
      className="rounded-xl p-6 scroll-mt-6"
      style={{ background: CARD_BG, border: '1px solid #2E3338' }}
    >
      <div className="flex justify-between items-center mb-5 flex-wrap gap-3">
        <h2 className="text-xl font-semibold" style={{ color: '#F6F9FD' }}>{chain.name}</h2>
        <a
          href={dashboardUrl}
          target="_blank"
      ...
      {error && (
        <div className="text-sm font-mono mb-3" style={{ color: '#FF294C' }}>Data unavailable</div>
      )}
      {!error && providers.length === 0 && (
        <div className="text-sm font-mono" style={{ color: '#656E80' }}>No data</div>
      )}
      ...
      {providers.length > 0 && (
        <div className="overflow-x-auto">
          <table className="w-full border-separate border-spacing-0">
            <thead>
              <tr className="text-[11px] uppercase tracking-wider font-mono"
                style={{ color: '#656E80' }}>
                <th className="sticky left-0 text-left font-normal py-2 pr-6"
                  style={{ background: CARD_BG }}>Provider</th>
                ...
                <th className="text-left font-normal pl-4 whitespace-nowrap">Regions, P95 ms</th>
              </tr>
            </thead>
            <tbody>
              {providers.map((p, i) => {
                ...
                return (
                  <tr key={p.name}
                    ...
                    style={{ borderTop: '1px solid #2E3338' }}>
                    <td className="sticky left-0 py-2.5 pr-6 font-mono"
                      style={{ background: CARD_BG }}>
                      <span style={{ color: isLeader ? '#F6F9FD' : '#8D95A5' }}>{p.name}</span>
                      {isLeader && (
                    ...
                    <td className="text-right px-3 py-2.5 font-mono tabular-nums text-[13px]"
                      style={{ color: avail != null ? availColor(avail) : '#656E80' }}>
                      {avail != null ? `${avail.toFixed(2)}%` : '—'}
                    </td>
                    <td className="text-right px-3 py-2.5 font-mono tabular-nums text-[15px] font-semibold"
                      style={{ color: p95ms != null ? p95Color(p95ms) : '#656E80' }}>
                      {p95ms ?? '—'}
                    </td>
                    <td className="text-right px-3 py-2.5 font-mono tabular-nums text-[13px]"
                      style={{ color: '#656E80' }}>
                    ...
                    <td className="px-3 py-2.5">
                      <Sparkline
                        ...
                        width={60}
                        height={24}
                        stroke={isLeader ? '#25B15F' : '#73808C'}
                        strokeWidth={1.25}
                      />
                    </td>
                    <td className="pl-4 py-2.5 font-mono tabular-nums text-[12px] whitespace-nowrap"
                      style={{ color: '#8D95A5' }}>
